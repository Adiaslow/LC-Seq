...
class StandardOutput(PipelineComponent):
                ...
                if encoding.chromatogram.peaks:
                    ...
                    if encoding.chromatogram.peaks:
                        ...
                        peptide.properties['picked_peak_metrics'] = {
                            ...
                            'gaussian_residuals': float(peak.properties.get('gaussian_residuals', 0.0)),
                            'gaussian_fit_amplitude': float(peak.properties.get('gaussian_fit_params', {}).get('amplitude', 0.0)),
                            'gaussian_fit_mean': float(peak.properties.get('gaussian_fit_params', {}).get('mean', 0.0)),
                            'gaussian_fit_sigma': float(peak.properties.get('gaussian_fit_params', {}).get('sigma', 0.0))
                        }
                    break
        return peptide
    ...
    def _save_results(self, output_data: Dict[str, Any]) -> None:
        """Save results to a YAML file with '_results' appended to the original filename."""
    ...
    def process_peptide(self, input_data: SinglePeptideInput) -> SinglePeptideInput:
        ...
        self.logger.info(f"Outputting results for peptide: {input_data.peptide.sequence_str}")
    ...
    def process_peptide_set(self, input_data: PeptideSetInput) -> PeptideSetInput:
        ...
        self.logger.info(f"Outputting results for peptide set: {len(input_data.peptides)} peptides")
