...
class GPPPeakAnalyzer(PipelineComponent):
    ...
    def analyze_peak(self, peak: Peak, chrom: Chromatogram) -> Peak:
        ...
        try:
            ...
            corrected_intensities = chrom.properties.get('corrected_intensities', chrom.intensities[peak_slice])
            ...
            try:
                window_extension = (peak.end_time - peak.start_time)  # Extend by one peak width on each side
                ...
                fit_intensities = chrom.properties.get('corrected_intensities', chrom.intensities)[fit_slice]

                p0 = [
                    peak.apex_intensity,  # Amplitude
                    peak.apex_time,       # Mean
                    max((peak.end_time - peak.start_time) / 2.355, self.config.min_std)  # Sigma (FWHM/2.355)
                ]
                ...
                gaussian_residuals = np.sqrt(np.mean(
        ...
        for encoding in input_data.peptide.encodings:
            if encoding.chromatogram is not None and encoding.chromatogram.peaks:
                for i, peak in enumerate(encoding.chromatogram.peaks):
                    encoding.chromatogram.peaks[i] = self.analyze_peak(peak, encoding.chromatogram)
        return input_data
