# src/lcseq/pipeline/components/standard_peak_detector.py
import logging
...
class PeakDetectionStats:
    ...
    excluded_by_prominence: int = 0
    excluded_by_width: int = 0
    excluded_by_distance: int = 0

class StandardPeakDetector(PipelineComponent):
    def __init__(self, config: StandardPeakDetectorConfig = None): # type: ignore
        self.config = config or StandardPeakDetectorConfig()
    ...
    def find_peaks(self, chrom: Chromatogram) -> Chromatogram:
        ...
        while iteration < self.config.max_threshold_iterations:
            ...
            if len(peak_indices) == 0:
                ...
                height_peaks, _ = sp_find_peaks(chrom.intensities, height=thresholds['height'])
                prominence_peaks, _ = sp_find_peaks(chrom.intensities, prominence=thresholds['prominence'])
                width_peaks, _ = sp_find_peaks(chrom.intensities, width=thresholds['width'])
                distance_peaks, _ = sp_find_peaks(chrom.intensities, distance=thresholds['distance'])

                stats.excluded_by_height = len(trial_peaks) - len(height_peaks)
                stats.excluded_by_prominence = len(trial_peaks) - len(prominence_peaks)
                ...
                if max_exclusions == stats.excluded_by_height:
                    ...
                    self.logger.info(f"Relaxing height threshold to {thresholds['height']}")
                elif max_exclusions == stats.excluded_by_prominence:
                    thresholds['prominence'] *= (1 - relaxation)
                    self.logger.info(f"Relaxing prominence threshold to {thresholds['prominence']}")
                elif max_exclusions == stats.excluded_by_width:
                    thresholds['width'] *= (1 - relaxation)
                    self.logger.info(f"Relaxing width threshold to {thresholds['width']}")
                elif max_exclusions == stats.excluded_by_distance:
                    thresholds['distance'] = max(1, int(thresholds['distance'] * (1 - relaxation)))
                    self.logger.info(f"Relaxing distance threshold to {thresholds['distance']}")
            ...
            for i, idx in enumerate(peak_indices):
                peak = Peak(
                    start_time=float(chrom.times[int(peak_properties['left_bases'][i])]),
                    apex_time=float(chrom.times[idx]),
                    end_time=float(chrom.times[int(peak_properties['right_bases'][i])]),
                    start_intensity=float(chrom.intensities[int(peak_properties['left_bases'][i])]),
                    apex_intensity=float(chrom.intensities[idx]),
                    end_intensity=float(chrom.intensities[int(peak_properties['right_bases'][i])]),
                    properties={
        ...
        self.logger.info(f"Peak detection completed after {stats.iterations} iterations")
        if stats.iterations > 1:
            self.logger.info(f"Thresholds were adjusted from {stats.initial_thresholds} to {stats.final_thresholds}")

        return chrom

    def process_peptide(self, input_data: SinglePeptideInput) -> SinglePeptideInput:
        """Process a single peptide."""
        self.logger.info(f"Detecting peaks for peptide: {input_data.peptide.sequence_str}")
    ...
    def process_peptide_set(self, input_data: PeptideSetInput) -> PeptideSetInput:
        ...
        self.logger.info(f"Detecting peaks for peptide set: {len(input_data.peptides)} peptides")
    ...
    def _calculate_width_threshold(self, chrom: Chromatogram) -> float:
        ...
        roughness = chrom.properties.get('baseline_roughness', self.config.min_roughness_factor)

        roughness_factor = max(
            self.config.min_roughness_factor,
            min(roughness * self.config.roughness_scale, self.config.max_roughness_factor)
        )
    ...
    def _calculate_window_length(self, chrom: Chromatogram) -> int:
        ...
        sampling_rate = (chrom.times[-1] - chrom.times[0]) / len(chrom.times)
        roughness = chrom.properties.get('baseline_roughness', self.config.min_window_roughness_factor)

        roughness_factor = max(
            self.config.min_window_roughness_factor,
            min(roughness * self.config.window_roughness_scale, self.config.max_window_roughness_factor)
        )
        window_length = max(int(roughness_factor / sampling_rate), self.config.min_window_points)
