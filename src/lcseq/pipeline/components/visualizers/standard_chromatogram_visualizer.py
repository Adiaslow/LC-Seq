...
class StandardChromatogramVisualizer(PipelineComponent):
    ...
    def visualize_chromatogram(self, chrom: Chromatogram, peptide_name: str) -> None:
        ...
        if "corrected_intensities" in chrom.properties:
            plt.plot(chrom.times, chrom.properties["corrected_intensities"],
                     linewidth=self.config.line_width,
        ...
        gaussian_fit_key = 'corrected_gaussian_fit_params' if self.config.plot_corrected_gaussians else 'gaussian_fit_params'
        if gaussian_fit_key in peak.properties:
            params = peak.properties[gaussian_fit_key]
            ...
            plt.plot(x_fit, y_fit,
                     ...
                     color='orange',
                     label='Corrected Gaussian Fit' if self.config.plot_corrected_gaussians else 'Gaussian Fit')
