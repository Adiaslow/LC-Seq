...
@pytest.fixture
def peptide_set_input(peptide_encodings):
    """Create PeptideSetInput from all valid peptide encodings."""
    peptides = []
    for encoding in peptide_encodings:
        peptide = Peptide(sequence=encoding.blocks, properties=encoding.properties)
        peptide.add_encoding(encoding)
...
def test_peptide_set_processing(peptide_set_input):
    ...
    # Test that each peptide has been processed
    for peptide in result.peptides:
        assert len(peptide.encodings) > 0
        for encoding in peptide.encodings:
            assert encoding.chromatogram is not None
